...
TJugadorsEquip* evaluateThreadFunc(void* arguments)
{
	...
	TJugadorsEquip *ThreadMillorEquip = malloc(sizeof(TJugadorsEquip));
	TEquip equip;
	int MaxPuntuacio=-1;
	...
	for (equip=args->first;equip<=args->end;equip++)
	{
		...
		if (PuntuacioEquip(jugadors)>MaxPuntuacio && CostEquip(jugadors)<args->PresupostFitxatges)
		{
	...
	free(arguments);
	//Return value
	pthread_exit(ThreadMillorEquip);
}
